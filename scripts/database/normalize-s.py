...
def convert_roman_numerals_to_numbers(text: str) -> str:
    """
    単独のローマ数字を算用数字へ変換する。
    ...
    def replace_roman(match: re.Match) -> str:
        ...
        return prefix + ROMAN_NUMBER_MAP[roman]
...
def get_maker_words() -> set[str]:
    ...
    for maker_name in MAKER_SLUG_MAP:
        ...
        if normalized_name:
            maker_words.add(normalized_name)
...
def update_normalized_machine_names(
    ...
    """
    categoryがslotのレコードだけを正規化して保存する。
